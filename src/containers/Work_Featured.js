...
class Work_Featured extends React.Component {
  ...
  handleScroll(e) {
    ...
    // console.log(window.innerHeight);
    if (
      checksection.getBoundingClientRect().top <= window.innerHeight * 0.75 &&
      checksection.getBoundingClientRect().top > 0
    ) {
      //console.log('showing');
      this.setState({ hudReveal: true, cardReveal: true });
          ...
          <Section_Content_Wrapper pagename={" "}>
            <div className="section_content_wrapper__cards">
              ...
              <div className="section_container__footer">
                ...
                <Link className="cta-button" to="/work">
                  VIEW ALL
                </Link>
                <object
                  className="decorator_right"
                  data={Decorator_More_Right}
                />
              </div>
            </div>
